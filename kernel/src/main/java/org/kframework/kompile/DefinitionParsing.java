...
public class DefinitionParsing {
    ...
    protected Definition resolveConfigBubbles(Definition definition, Module defaultConfiguration) {
        ...
        Definition result;
        try {
            Definition defWithParsedConfigs = resolveBubbles(definitionWithMapForConfig, false);
            throwExceptionIfThereAreErrors();
            result = resolveConfigBubbles(defWithParsedConfigs);
        } catch (KEMException e) {
            errors.add(e);
            throwExceptionIfThereAreErrors();
            throw new AssertionError("should not reach this statement");
    ...
    private Definition resolveBubbles(Definition def, boolean isRule) {
        ...
        // TODO: link with parallels scanner generation -> parser generation -> parse
